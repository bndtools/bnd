...
public class Project extends Processor {
        ...
        try {
            if (!preparedPaths) {
                ...
                try {
                    ...
                    output = getFile(getProperty("bin", "bin"))
                            .getAbsoluteFile();
                    if (!output.exists()) {
                        output.mkdirs();
                    ...
                    doPath(buildpath, dependencies, parseBuildpath(),
                            bootclasspath);
                    doPath(runpath, dependencies, parseTestpath(),
                            bootclasspath);
     ...
     * @return
     */
    public List<Container> getBundles(int strategyx, String spec)
            throws Exception {
        ...
        try {
            for (Iterator<Map.Entry<String, Map<String, String>>> i = bundles
                ...
                if (versionRange != null) {
                    if (versionRange.equals("latest")
                            || versionRange.equals("snapshot")) {
                        found = getBundle(bsn, versionRange, strategyx, attrs);
                    }
                }
                if (found == null) {
                    if (versionRange != null
                            && (versionRange.equals("project") || versionRange
                                    .equals("latest"))) {
     ...
     */
    public Container getBundle(String bsn, String range, int strategyx,
     ...
     * @throws Exception
     */
    private Container getBundleFromProject(String bsn, Map<String, String> attrs)
            throws Exception {
        String pname = bsn;
        while (true) {
        ...
        for (String bsn : parts) {
            ...
            if (container.getError() != null) {
                error("The ${repo} macro could not find " + bsn
                        + " in the repo, because " + container.getError()
                        + "\n" + "Repositories     : "
    ...
    private void add(List<String> paths, Container container) throws Exception {
        if (container.getError() == null) {
            ...
            error("Loading library file: %s getting %s", container
                    .getBundleSymbolicName(), container.getError());
        }
    }
    ...
    public boolean test() throws Exception {
        ...
        if (jars != null) {
            ...
            error("No %s set nor any JARs created in this project %s",
                    TESTBUNDLES, this);
            return false;
        }
    ...
    public Jar getValidJar(URL url) throws Exception {
        ...
        try {
            Jar jar = new Jar(url.getFile().replace('/', '.'), in, System
                    .currentTimeMillis());
    ...
    public Jar getValidJar(Jar jar, String id) throws Exception {
        ...
        if (manifest == null) {
            ...
            b.addClasspath(jar);
            b.setProperty("Bnd-Message", "Wrapped from " + id
    ...
    public Collection<Container> getDeliverables() throws Exception {
        List<Container> result = new ArrayList<Container>();
        Collection<Builder> builders = getSubBuilders();
     ...
     * @throws Exception
     */
    public Container getDeliverable(String bsn, Map<String, String> attrs)
            throws Exception {
        Collection<Builder> builders = getSubBuilders();
        for (Builder sub : builders) {
     ...
     * @throws Exception
     */
    public Collection<Builder> getSubBuilders() throws Exception {
        ProjectBuilder pb = getBuilder(null);
        return pb.getSubBuilders();
    }
