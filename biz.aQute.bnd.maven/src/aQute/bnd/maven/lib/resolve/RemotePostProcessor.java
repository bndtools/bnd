...
public class RemotePostProcessor implements PostProcessor {
    ...
    public RemotePostProcessor(RepositorySystemSession session, RepositorySystem system, MetadataReader metadataReader, LocalURLs localURLs) {
    ...
    private ArtifactResult postProcessSnapshot(ArtifactRequest request, Artifact artifact) throws MojoExecutionException {
        ...
        for (RemoteRepository repository : request.getRepositories()) {
            ...
            MetadataRequest mr = new MetadataRequest().setRepository(repository)
                .setMetadata(new DefaultMetadata(artifact.getGroupId(), artifact.getArtifactId(), artifact.getVersion(), "maven-metadata.xml", SNAPSHOT));
            ...
            for (MetadataResult metadataResult : system.resolveMetadata(newSession, singletonList(mr))) {
                if (metadataResult.isResolved()) {
                    ...
                    try {
                        ...
                        if (versioning == null || versioning.getSnapshotVersions() == null || versioning.getSnapshotVersions()
                            .isEmpty()) {
                            continue;
                        }
                    ...
                    try {
                        ArtifactResult result = system.resolveArtifact(newSession, new ArtifactRequest().setArtifact(fullVersionArtifact)
    ...
    private ArtifactResult postProcessRelease(ArtifactRequest request, Artifact artifact) throws MojoExecutionException {
        ...
        for (RemoteRepository repository : request.getRepositories()) {
            ...
            // Find the snapshot metadata for the module
            MetadataRequest mr = new MetadataRequest().setRepository(repository)
                .setMetadata(new DefaultMetadata(artifact.getGroupId(), artifact.getArtifactId(), null, "maven-metadata.xml", RELEASE));
                ...
                if (metadataResult.isResolved()) {
                    try {
                        ...
                        if (versioning == null || versioning.getVersions() == null || versioning.getVersions()
                            ...
                            ArtifactResult result = system.resolveArtifact(newSession, new ArtifactRequest().setArtifact(artifact)
                                .addRepository(repository));
                            if (result.isResolved()) {
                                File toUse = new File(session.getLocalRepository()
                                    .getBasedir(),
