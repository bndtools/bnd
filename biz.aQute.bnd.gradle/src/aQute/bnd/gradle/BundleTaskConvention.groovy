...
class BundleTaskConvention {
	...
	BundleTaskConvention(BundleTaskExtension extension, org.gradle.api.tasks.bundling.Jar task) {
	...
	public void setBndfile(String file) {
		extension.getBndfile().value(task.getProject().getLayout().getProjectDirectory().file(file))
	}

	public void setBndfile(Object file) {
		extension.getBndfile().set(task.getProject().file(file))
	}
