...
public class MustacheTemplateEngine implements TemplateEngine {
	...
	@Override
	public ResourceMap generateOutputs(ResourceMap inputs, Map<String, List<Object>> parameters,
				...
				switch (source.getType()) {
					...
					case File :
						if (settings.preprocessMatch.matches(inputPath)) {
							// This file should be processed with the template
							// engine
							try (Reader reader = IO.reader(source.getContent(), source.getTextEncoding())) {
	...
	private static Properties readDefaults(ResourceMap inputs) throws IOException {
		...
		if (defaultsResource != null) {
			if (defaultsResource.getType() != ResourceType.File)
				throw new IllegalArgumentException(
					String.format("Default properties resource %s must be a file; found resource type %s",
						DEFAULT_PROPERTIES, defaultsResource.getType()));
			try (Reader reader = new InputStreamReader(defaultsResource.getContent(),
				defaultsResource.getTextEncoding())) {
				props.load(reader);
			}
		}
		return props;
