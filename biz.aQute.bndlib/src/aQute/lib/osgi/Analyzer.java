...
public class Analyzer extends Processor {
    ...
    public Manifest calcManifest() throws Exception {
        ...
        Set<Instruction> matchers = Instruction.replaceWithInstruction(removes)
                .keySet();

        Collection<Object> toBeRemoved = Instruction.select(matchers, main
                .keySet());
    ...
    public boolean checkClass(String interfaceName) {
        ...
        if (n > 0 && n + 1 < interfaceName.length()
            ...
            return checkClass(interfaceName.substring(0, n) + '$'
                    + interfaceName.substring(n + 1));
        }
        return false;
    }
    ...
    Map<String, Map<String, String>> addExportsToImports(
        ...
        for (Map.Entry<String, Map<String, String>> packageEntry : exports
            ...
            if (noimport == null || !noimport.equalsIgnoreCase("true")) {
                if (!usesInternal(exports,packageName)) {
    ...
    /** 
     * Check if the given package uses an internal package. An internal package
     * is defined as being contained and exported.
     *  
     * @param packageName The name of the package
     * @return true if the package uses internal code, false otherwise
     */
    private boolean usesInternal(Map<String, Map<String, String>> exports, String packageName) {
        Set<String> set = Create.copy(uses.get(packageName));
            
        set.removeAll(exports.keySet());
        set.retainAll(contained.keySet());
        if ( !set.isEmpty()) {
            trace("Not importing exported package %s because it refers to private package %s", packageName, set);
            return true;
        }
        return false;
    }
    ...
    private void augmentVersion(Map<String, String> currentAttributes,
        ...
        try {
            ...
            if (importRange != null) {
                ...
                importRange = getVersionPolicy(isTrue(currentAttributes
                        .get(IMPLEMENTED_DIRECTIVE)));
    ...
    public void close() {
        if (diagnostics) {
            ...
            for (Jar jar : getClasspath()) {
                ...
                for (Map.Entry<String, Map<String, Resource>> entry : dirs
                        .entrySet()) {
            ...
            for (String path : bundleClasspath.keySet()) {
                ...
                if (resource != null) {
                    ...
                    if (dot.getDirectories().containsKey(path)) {
                        analyzeJar(dot, Processor.appendPath(path) + "/",
                                classSpace, contained, referred, uses, hide);
                    } else {
    ...
    private void parsePackageInfoClass(final Clazz clazz,
                    ...
                    if (!info.containsKey(Constants.VERSION_ATTRIBUTE)) {
                        ...
                        if (version != null) {
                            if (Verifier.VERSION.matcher(version).matches())
                                info.put(VERSION_ATTRIBUTE, getReplacer()
                                        .process(version));
                            else
                                error(
                                        "Export annotatio in %s has invalid version info: %s",
                                        clazz, version);
                        }
                    }

                    // Check mandatory attributes
                    Map<String, String> attrs = doAttrbutes((Object[]) a
                            .get(Export.MANDATORY), clazz, getReplacer());
                    if (!attrs.isEmpty()) {
                        info.putAll(attrs);
                        info.put(MANDATORY_DIRECTIVE, Processor.join(attrs
                                .keySet()));
                    }

                    // Check optional attributes
                    attrs = doAttrbutes((Object[]) a.get(Export.OPTIONAL),
                            clazz, getReplacer());
                    if (!attrs.isEmpty()) {
                        info.putAll(attrs);
                    }
