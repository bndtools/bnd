...
public class Clazz {
    ...
    static public class FieldDef implements Comparable<FieldDef> {
        ...
        public boolean equals(Object other) {
            ...
            return clazz.equals(m.clazz) && name.equals(m.name)
                    && descriptor.equals(m.descriptor);
        }
    ...
    static public class MethodDef extends FieldDef {
        public MethodDef(int access, String clazz, String method,
                String descriptor) {
        ...
        private int printParameter(StringBuilder sb, CharSequence descriptor,
                int index) {
        ...
        private int reference(StringBuilder sb, CharSequence descriptor,
                int index) {
            int n = sb.length();
            int lastSlash = n;
            while (index < descriptor.length()
    ...
    Set<String> parseClassFile(DataInputStream in) throws IOException {
        ...
        try {
            ...
            for (int i = 0; i < fieldsCount; i++) {
                ...
                if (cd != null)
                    cd.field(new FieldDef(access_flags, className, name,
            ...
            for (int i = 0; i < methodCount; i++) {
                ...
                if (cd != null)
                    cd.method(new MethodDef(access_flags, className, name,
                            descriptor));
     ...
     */
    private int findMethodReference(String clazz, String methodname,
            String descriptor) {
    ...
    public boolean is(QUERY query, Instruction instr, Analyzer analyzer)
            throws Exception {
        switch (query) {
        case ANY:
            return true;
    ...
    public static void getImplementedPackages(Set<String> implemented,
            Analyzer analyzer, Clazz clazz) throws Exception {
        if (clazz.interfaces != null) {
            for (String interf : clazz.interfaces) {
                ...
                if (c != null) {
                    if (!(c.annotations != null && c.annotations
                            .contains(UsePolicy.RNAME)))
