...
public class Syntax implements Constants {
	...
	static final Syntax[]					syntaxes				= new Syntax[] {
			new Syntax(".bnd", "Home directory usage (~/.bnd) in bnd.", null, null, null, new Syntax[] {
			...
			new Syntax(BUNDLE_CONTRIBUTORS, "Defines the people that contrbuted to this bundle", BUNDLE_CONTRIBUTORS
					+ ": Peter.Kriens@aQute.biz;name='Peter Kriens Ing';organization=aQute;organizationUrl='http://www.aQute.biz';roles=ceo;timezone=+1",
					null, null,
					new Syntax("name", "The display name of the developer", "name='Peter Kriens'", null, null),
			...
			new Syntax(BUNDLE_ICON, "The optional " + BUNDLE_ICON
					...
					new Syntax("size", "Icons size in pixels, e.g. 64.", "size=64", "16,32,48,64,128",
			...
			new Syntax(BUNDLE_SYMBOLICNAME, "The " + BUNDLE_SYMBOLICNAME
					...
					new Syntax(BLUEPRINT_WAIT_FOR_DEPENDENCIES_ATTRIBUTE, "", "", "true,false",
			...
			new Syntax(IMPORT_PACKAGE, "The " + IMPORT_PACKAGE
					...
					new Syntax(RESOLUTION_DIRECTIVE,
							...
							RESOLUTION_DIRECTIVE + "=optional", "mandatory,optional",
			...
			new Syntax(MAKE,
					...
					new Syntax("recipe", "Recipe for the plugin, can use back references.", "recipe=\"bnd/$1.bnd\"",
							"bnd", null)),
			...
			new Syntax(PLUGIN, "Define the plugins.",
					PLUGIN + "=aQute.lib.spring.SpringComponent,aQute.lib.deployer.FileRepo;location=${repo}", null,
					null),
			...
			new Syntax(PACKAGE, "Defines the options for packaging", PACKAGE + "=<packaging-type>", null, null),
