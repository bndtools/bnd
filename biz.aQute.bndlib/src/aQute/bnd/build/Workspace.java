...
public class Workspace extends Processor {
	...
	public Workspace(File dir) throws Exception {
		...
		addBasicPlugin(new CachedFileRepo());
	}
	...
	void explodeJar(InputStream in, File dir) throws Exception {
		try {
			JarInputStream jin = new JarInputStream(in);
			JarEntry jentry = jin.getNextJarEntry();
			while (jentry != null) {
				if (!jentry.isDirectory()) {
					File dentry = getFile(CACHEDIR + "/" + jentry.getName());
					if (!dentry.isFile() || dentry.lastModified() < jentry.getTime()) {
						FileOutputStream out = new FileOutputStream(dentry);
						try {
							copy(jin, out);
						} finally {
							out.close();
	...
	class CachedFileRepo extends FileRepo {
		...
		CachedFileRepo() {
			super("cache",getFile(CACHEDIR), true);
		}
		...
		protected void init() throws Exception {
			if (!inited) {
				...
				InputStream in = getClass().getResourceAsStream("/embedded-repo.jar");
				if ( in != null )
					explodeJar(in, root);
			}
		}
