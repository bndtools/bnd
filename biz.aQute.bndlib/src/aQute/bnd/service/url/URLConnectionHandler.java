 ...
 * <ul> <li>{@link BndAuthentication} — Authenticates using bnd's built in
 * private key (see {@link Settings} <li>{@link BasicAuthentication} — Http
 * Basic Authentication <li>{@link HttpsVerification} — Can add certificats for
 * verification of Https or disable verification <li>{@link ConnectionSettings}
 * — Can set arbitrary headers on an Http(s) connection </ul> URLConnection are
 * allows matched to the URL. This {@link URLConnectionHandler#MATCH} is a comma
 * separated list of Glob expressions that must match the canonical URL string
 * representation.
 */
public interface URLConnectionHandler {

	/**
	 * Configuration property for the matcher. A comma separated list of {@link
