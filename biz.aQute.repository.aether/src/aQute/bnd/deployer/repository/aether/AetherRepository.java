	...
	public void setProperties(Map<String,String> props) throws Exception {
		...
		if (mainUrlStr == null)
			throw new IllegalArgumentException(
					String.format("Attribute '%s' must be set on '%s' plugin.", PROP_MAIN_URL, getClass().getName()));
		try {
		...
		if (cachePath != null) {
			...
			if (!cacheDir.isDirectory()) {
				...
				throw new IllegalArgumentException(
						String.format("Cache path '%s' does not exist, or is not a directory", canonicalPath));
			}
		}
	}
	...
	protected final synchronized void init() throws Exception {
		...
		locator.setErrorHandler(new ErrorHandler() {
			...
			public void serviceCreationFailed(Class< ? > type, Class< ? > impl, Throwable exception) {
				if (reporter != null)
					reporter.error("Service creation failed for type %s using impl %s: %s", type, impl,
			...
			session.setTransferListener(new AbstractTransferListener() {
				...
				public void transferSucceeded(TransferEvent event) {
					...
					if (event.getRequestType() == RequestType.PUT && tmpFile.equals(resource.getFile())) {
						...
						System.out.println(
								"UPLOADED to: " + URI.create(resource.getRepositoryUrl() + resource.getResourceName()));
					}
				}
