...
public class BundleTaskExtension {
	...
	private class BuildAction implements Action<Task> {
		...
		private org.gradle.api.java.archives.Manifest mergeManifest(Manifest builtManifest) {
			 ...
			 */
			return BuiltManifest.proxy(builtManifest);
		}
	...
	/**
	 * A read-only dynamic proxy over the attributes generated by the bnd
	 * builder. The proxy avoids directly implementing Gradle's
	 * {@link org.gradle.api.java.archives.Attributes} ProviderType interface,
	 * which Gradle may extend in a compatible release. Methods inherited from
	 * {@link Map} are delegated to an unmodifiable {@link AttributesMap}; future
	 * default methods are invoked on the proxy and unsupported methods fail
	 * explicitly.
	 */
	static final class BuiltAttributes implements InvocationHandler {
		private static final Map<Method, Method> methods = attributeMethods();
		private final Map<String, Object> attributes;

		private static Map<Method, Method> attributeMethods() {
			Map<Method, Method> methods = new HashMap<>();
			for (Method method : org.gradle.api.java.archives.Attributes.class.getMethods()) {
				try {
					Method mapMethod = Map.class.getMethod(method.getName(), method.getParameterTypes());
					methods.put(method, mapMethod);
				} catch (NoSuchMethodException e) {
					// A future Attributes method is handled by invoke.
				}
			}
			return Collections.unmodifiableMap(methods);
		}

		private BuiltAttributes(java.util.jar.Attributes attributes) {
			this.attributes = new AttributesMap(attributes);
		}

		static org.gradle.api.java.archives.Attributes proxy(java.util.jar.Attributes attributes) {
			return (org.gradle.api.java.archives.Attributes) Proxy.newProxyInstance(
				BuiltAttributes.class.getClassLoader(), new Class<?>[] {
					org.gradle.api.java.archives.Attributes.class
				}, new BuiltAttributes(attributes));
		}
		...
		public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
			if (method.getDeclaringClass() == Object.class) {
				...
				default :
					throw new UnsupportedOperationException("read-only attributes: " + method);
				}
			}
			Method mapMethod = methods.get(method);
			if (mapMethod != null) {
				try {
					return mapMethod.invoke(attributes, args);
	 ...
	 * <p>
	 * {@link org.gradle.api.java.archives.Manifest} is a Gradle
	 * {@code @ProviderType}: it is not meant to be implemented directly by
	 * consumers, since a future Gradle version can add a new abstract method
	 * to it in a minor release. A class compiled against an older version of
	 * the interface would then fail with an {@link AbstractMethodError} once
	 * Gradle invoked the missing method. A dynamic {@link Proxy} avoids that
	 * risk: the proxy class is generated at run time against whatever version
	 * of the interface Gradle loads, so it always has every method: unknown or
	 * future methods land in the {@code default} case below instead of
	 * crashing with a linkage error.
	 */
	static final class BuiltManifest implements InvocationHandler {
		private static final Method	getAttributesMethod		= manifestMethod("getAttributes");
		private static final Method	getSectionsMethod		= manifestMethod("getSections");
		private static final Method	getEffectiveManifestMethod	= manifestMethod("getEffectiveManifest");
		...
		private static Method manifestMethod(String name, Class<?>... parameterTypes) {
			try {
				return org.gradle.api.java.archives.Manifest.class.getMethod(name, parameterTypes);
			} catch (NoSuchMethodException e) {
				throw new ExceptionInInitializerError(e);
			}
		}

		private BuiltManifest(Manifest builtManifest) {
			this.attributes = BuiltAttributes.proxy(builtManifest.getMainAttributes());
			Map<String, org.gradle.api.java.archives.Attributes> sections = new LinkedHashMap<>();
			builtManifest.getEntries()
				.forEach((section, attrs) -> sections.put(section, BuiltAttributes.proxy(attrs)));
			this.sections = Collections.unmodifiableMap(sections);
		}

		static org.gradle.api.java.archives.Manifest proxy(Manifest builtManifest) {
			return (org.gradle.api.java.archives.Manifest) Proxy.newProxyInstance(BuiltManifest.class.getClassLoader(),
				new Class<?>[] {
					org.gradle.api.java.archives.Manifest.class
				}, new BuiltManifest(builtManifest));
		}
		...
		public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
			if (method.getDeclaringClass() == Object.class) {
				...
				case "equals" :
					return proxy == args[0];
				case "toString" :
					return "BuiltManifest[attributes=" + attributes + ", sections=" + sections + "]";
				default :
					throw new UnsupportedOperationException("read-only manifest: " + method);
				}
			}
			if (method.equals(getAttributesMethod)) {
				return attributes;
			}
			if (method.equals(getSectionsMethod)) {
				return sections;
			}
			if (method.equals(getEffectiveManifestMethod)) {
				return proxy;
			}
			if (method.isDefault()) {
				return InvocationHandler.invokeDefault(proxy, method, args);
