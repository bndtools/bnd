...
class BundleTaskExtension {
	...
	void buildBundle() {
		...
		FileCollection sourcepath = allSource.filter({ File file ->
			file.exists()
		})
		// create Builder
		...
		Builder builder = new Builder(new Processor(gradleProperties, false))
		try {
			// load bnd properties
			File temporaryBndFile = File.createTempFile("bnd", ".bnd", jarTask.getTemporaryDir())
			Writer writer = IO.writer(temporaryBndFile)
			try  {
