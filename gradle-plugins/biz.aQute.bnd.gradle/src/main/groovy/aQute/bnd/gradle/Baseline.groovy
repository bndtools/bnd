...
public class Baseline extends DefaultTask {
	...
	void baselineAction() {
		...
		try {
			...
			def infos = baseliner.baseline(newer, older, new Instructions(new Parameters(Strings.join(unwrap(getDiffpackages())), processor)))
					.stream()
					.sorted(Comparator.comparing({info -> info.packageName}))
			def bundleInfo = baseliner.getBundleInfo()
			Formatter f = new Formatter(report, "UTF-8", Locale.US)
			try {
				f.format("===============================================================%n")
				f.format("%s %s %s-%s",
						bundleInfo.mismatch ? "*" : " ",
