...
public class Resolve extends AbstractBndrun {
	 ...
	 */
	@Override
	protected Object createRun(def workspace, File bndrunFile) {
		File outputBndrunFile = unwrap(getOutputBndrun())
		if (!Objects.equals(outputBndrunFile, bndrunFile)) {
			Writer writer = IO.writer(outputBndrunFile)
			try {
	...
	protected void worker(def run) {
		...
		try {
			def result = run.resolve(isFailOnChanges(), isWriteOnChanges())
