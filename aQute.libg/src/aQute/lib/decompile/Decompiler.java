...
public class Decompiler {
    ...
    public Decompiler(String path) throws Exception {
        this(path, new FileInputStream(new File(path)));
    }

    public Decompiler(String path, InputStream in) throws IOException {
        this.path = path;
        this.in = in;
    }
        ...
        if ( (access & ACC_INTERFACE) != 0) {
        	...
        	if ( interfaces_count >0) {
                ...
                for (int i = 0; i < interfaces_count; i++) {
                    ...
                    ps.printf("%s%s", del, pool[interface_index]);
                    del = ", ";
                }        		
        	}
        	...
        	if ( zuper == null || zuper.equals("java/lang/Object")) {
        		ps.printf("extends %s", zuper);
        	}
        	if ( interfaces_count >0) {
        		String del = "implements ";
                for (int i = 0; i < interfaces_count; i++) {
                    int interface_index = in.readUnsignedShort();
                    ps.printf("%s%s", del, pool[interface_index]);
                    del = ", ";
    ...
    public static void main(String args[]) throws Exception {
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            if (!f.isFile())
                ...
                Decompiler cd = new Decompiler(args[i]);
                cd.dump(null);
            }
        }
    }
