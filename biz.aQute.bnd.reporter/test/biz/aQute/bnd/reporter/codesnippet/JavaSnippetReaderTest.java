...
public class JavaSnippetReaderTest extends TestCase {
  ...
  public void testOnlyContentMethod() {
    ...
    assertEquals(1, r.size());
    checkSnippet(r.get(0), null, "print", null, null, "// Comment\n"
        + "final MyClass c = new MyClass();\n" + "System.out.println(c.toString());");
  }
  ...
  public void testWithGroup() {
    ...
    assertEquals(1, r.size());
    assertEquals("test", r.get(0).getGroupName());
    assertEquals(null, r.get(0).getCodeSnippetProgram());
    assertEquals(null, r.get(0).getParentGroup());
    assertEquals("WithGroup", r.get(0).getCodeSnippetGroup().id);
    assertEquals(null, r.get(0).getCodeSnippetGroup().title);
    assertEquals(null, r.get(0).getCodeSnippetGroup().description);
    assertEquals(0, r.get(0).getCodeSnippetGroup().steps.size());
  }
  ...
  public void testWithParentGroup() {
    ...
    assertEquals(null, r.get(0).getCodeSnippetProgram());
    assertEquals(null, r.get(0).getParentGroup());
    assertEquals("WithParentGroup", r.get(0).getCodeSnippetGroup().id);
    assertEquals(null, r.get(0).getCodeSnippetGroup().title);
    assertEquals(null, r.get(0).getCodeSnippetGroup().description);
    assertEquals(0, r.get(0).getCodeSnippetGroup().steps.size());

    assertEquals(null, r.get(1).getGroupName());
    assertEquals(null, r.get(1).getCodeSnippetGroup());
    assertEquals("test", r.get(1).getParentGroup());
    assertEquals("WithParentGroup1", r.get(1).getCodeSnippetProgram().id);
    assertEquals(null, r.get(1).getCodeSnippetProgram().title);
    assertEquals(null, r.get(1).getCodeSnippetProgram().description);
    assertEquals("java", r.get(1).getCodeSnippetProgram().programmingLanguage);
    assertEquals("public abstract class WithParentGroup {\n" + "\n"
        + "  abstract public void print();\n" + "}", r.get(1).getCodeSnippetProgram().codeSnippet);
  }

  private void checkSnippet(final Snippet s, final String parentGroup, final String id,
      final String title, final String description, final String code) {
  ...
  private JavaSnippetReader getReader() {
    ...
    r.init((i) -> {
			final Integer count = idCache.put(i,
					Integer.valueOf(idCache.getOrDefault(i, Integer.valueOf(0)).intValue() + 1));
      if (count != null) {
        return i + count;
      } else {
        return i;
