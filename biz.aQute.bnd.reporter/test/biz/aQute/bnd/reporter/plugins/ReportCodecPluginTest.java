...
public class ReportCodecPluginTest {
  ...
  public void testJsonDeserialization() throws Exception {
    ...
    final Object entry =
        new JsonImportDeserializerPlugin().deserialyze(new ByteArrayInputStream(json.getBytes()));

    assertEquals("test", entry);
    assertArrayEquals(new String[] {"json"},
        new JsonImportDeserializerPlugin().getHandledExtensions());
  }
  ...
  public void testPropertiesDeserialization() throws Exception {
    ...
    prop = "";
    entry = new PropertiesImportDeserializerPlugin()
        .deserialyze(new ByteArrayInputStream(prop.getBytes()));
  ...
  public void testManifestDeserialization() throws Exception {
    ...
    out = new ByteArrayOutputStream();
    manifest.write(out);
    entry = new ManifestImportDeserializerPlugin()
        .deserialyze(new ByteArrayInputStream(out.toByteArray()));
    assertEquals(expected, entry);
    ...
    new JsonReportSerializerPlugin().serialize(toSerialize, out);
    assertEquals(
        new String(IO.read(IO.stream(Paths.get("testresources/manifestJsonResult.json")))).length(),
        new String(out.toByteArray()).length());
    assertEquals(new JsonImportDeserializerPlugin()
        .deserialyze(IO.stream(Paths.get("testresources/manifestJsonResult.json"))), toSerialize);
    assertArrayEquals(new String[] {"mf"},
        new ManifestImportDeserializerPlugin().getHandledExtensions());
  }
  ...
  public void testXmlDeserialization() throws Exception {
    ...
    xml =
        "<rootInFile><oneElement>oneValue</oneElement><oneElement>otherValue</oneElement></rootInFile>";
    entry = new XmlImportDeserializerPlugin().deserialyze(new ByteArrayInputStream(xml.getBytes()));
    assertEquals(objectExpected, entry);
    ...
    xml =
        "<rootInFile><oneElement>oneValue</oneElement>just a text<oneElement>otherValue</oneElement></rootInFile>";
    entry = new XmlImportDeserializerPlugin().deserialyze(new ByteArrayInputStream(xml.getBytes()));
    assertEquals(objectExpected, entry);
    ...
    xml =
        "<rootInFile><oneElement>oneValue</oneElement>just a text<oneElement>otherValue</oneElement>another text</rootInFile>";
    entry = new XmlImportDeserializerPlugin().deserialyze(new ByteArrayInputStream(xml.getBytes()));
    assertEquals(objectExpected, entry);
    ...
    objectExpected.put("_text", values);
    xml =
        "<rootInFile><oneElement>oneValue</oneElement>just a text<oneElement><otherElement>otherValue</otherElement><otherElement>otherValue2</otherElement><yetAnotherElement>otherValue3</yetAnotherElement></oneElement>another text</rootInFile>";
    entry = new XmlImportDeserializerPlugin().deserialyze(new ByteArrayInputStream(xml.getBytes()));
    assertEquals(objectExpected, entry);

    entry = new XmlImportDeserializerPlugin()
        .deserialyze(IO.stream(Paths.get("testresources/complex.xml")));
    ...
    assertArrayEquals(new String[] {"xml"},
        new XmlImportDeserializerPlugin().getHandledExtensions());
  }
}
