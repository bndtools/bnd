...
public class CommonInfoPluginTest extends TestCase {
  ...
  public void testFullProp() throws Exception {
    ...
    final CommonInfoDTO infoDto = plugin.extract(
        getWorkspace("ws-copyright", "test1", "ws-description", "test2", "ws-docURL", "test3",
            "ws-name", "test4", "ws-updateLocation", "test5", "ws-vendor", "test6",
            "ws-contactAddress", "test7", "ws-developers", "test8", "ws-icons", "test9",
            "ws-licenses", "test10", "ws-scm", "url=test11", "ws-version", "1.0.0"),
    ...
    assertEquals("test8", infoDto.developers.iterator().next().identifier);
    assertEquals("test9", infoDto.icons.iterator().next().url);
    assertEquals("test10", infoDto.licenses.iterator().next().name);
  ...
  public void testFullHeader() throws Exception {
    ...
    final CommonInfoDTO infoDto = plugin.extract(
        getWorkspace(Constants.BUNDLE_COPYRIGHT, "test1", Constants.BUNDLE_DESCRIPTION, "test2",
            Constants.BUNDLE_DOCURL, "test3", Constants.BUNDLE_NAME, "test4",
            Constants.BUNDLE_UPDATELOCATION, "test5", Constants.BUNDLE_VENDOR, "test6",
            Constants.BUNDLE_CONTACTADDRESS, "test7", Constants.BUNDLE_DEVELOPERS, "test8",
            Constants.BUNDLE_ICON, "test9", Constants.BUNDLE_LICENSE, "test10",
            Constants.BUNDLE_SCM, "url=test11", Constants.BUNDLE_VERSION, "1.0.0"),
        Locale.forLanguageTag("und"));
  ...
  public void testMixPropHeader() throws Exception {
    ...
    final CommonInfoDTO infoDto = plugin.extract(
        getWorkspace(Constants.BUNDLE_COPYRIGHT, "not", "ws-copyright", "test1",
            Constants.BUNDLE_DESCRIPTION, "test2", Constants.BUNDLE_DOCURL, "test3",
            Constants.BUNDLE_UPDATELOCATION, "test5", Constants.BUNDLE_VENDOR, "test6", "ws-vendor",
            "", Constants.BUNDLE_CONTACTADDRESS, "test7", Constants.BUNDLE_DEVELOPERS, "test8",
            Constants.BUNDLE_ICON, "test9", Constants.BUNDLE_LICENSE, "test10",
            Constants.BUNDLE_SCM, "url=test11", Constants.BUNDLE_VERSION, "1.0.0"),
        Locale.forLanguageTag("und"));
  ...
  private Workspace getWorkspace(final String... prop) throws Exception {
    final File wsFile = Files.createTempDirectory("bnd-ws").toFile();
    wsFile.deleteOnExit();

    final File cnf = Files.createDirectory(Paths.get(wsFile.getPath(), "cnf")).toFile();
    cnf.deleteOnExit();
    ...
    final Iterator<String> it = Arrays.asList(prop).iterator();
    while (it.hasNext()) {
