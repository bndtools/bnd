...
public class CodeSnippetPluginTest extends TestCase {
  ...
  private Project getProject() throws Exception {
    ...
    final File p1 = Files.createDirectory(Paths.get(ws.getBase().getPath(), "project")).toFile();
    p1.deleteOnExit();
  ...
  private Project getProjectWithSnippet(final boolean custom) throws Exception {
    ...
    final File test = Files.createDirectory(Paths.get(p.getBase().getPath(), "test")).toFile();
    test.deleteOnExit();

    final File examples = Files.createDirectory(Paths.get(test.getPath(), dirName)).toFile();
    examples.deleteOnExit();

    final File code = new File(p.getBase() + "/test/" + dirName, "Test.java");
    code.createNewFile();

    IO.write(new String("package biz.aQute.bnd.reporter.codesnippet.examples.first;\n" + "\n"
