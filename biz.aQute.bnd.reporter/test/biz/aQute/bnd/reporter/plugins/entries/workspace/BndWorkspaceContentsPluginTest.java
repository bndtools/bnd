...
public class BndWorkspaceContentsPluginTest extends TestCase {
  ...
  public void testWorkspaceWithProjects() throws Exception {
    ...
    final List<?> result =
        (List<?>) plugin.extract(getWorkspace("test1", "test2"), Locale.forLanguageTag("und"));
  ...
  private Workspace getWorkspace(final String project1, final String project2) throws Exception {
    ...
    final File p1 = Files.createDirectory(Paths.get(ws.getBase().getPath(), project1)).toFile();
    p1.deleteOnExit();
    ...
    final File p2 = Files.createDirectory(Paths.get(ws.getBase().getPath(), project2)).toFile();
    p2.deleteOnExit();
