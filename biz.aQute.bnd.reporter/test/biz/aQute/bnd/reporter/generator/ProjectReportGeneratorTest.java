...
public class ProjectReportGeneratorTest extends TestCase {
	...
	private Project getProjectPlus()
	throws Exception {
	...
	public void testProjectReportModel()
	throws Exception {
		...
		try (ProjectReportGenerator rg = new ProjectReportGenerator(ws)) {
			...
			a.clear();
			ws.setProperty(Constants.REPORT_PROJECT_MODEL, "excludes='settings'");
			GeneratorAsserts.verify(rg, 0, a, 0);
			
			a.clear();
			ws.setProperty(Constants.REPORT_PROJECT_MODEL, "imports='notFound'");
			GeneratorAsserts.verify(rg, 1, a, 0);
			
			a.clear();
			ws.setProperty(Constants.REPORT_PROJECT_MODEL, "imports='bnd.bnd:prop:properties'");
			GeneratorAsserts.verify(rg, 2, a, 0);
			
			a.clear();
			ws.setProperty(Constants.REPORT_PROJECT_MODEL, "imports='bnd.bnd::properties'");
			GeneratorAsserts.verify(rg, 2, a, 0);
			
			a.clear();
			ws.setProperty(Constants.REPORT_PROJECT_MODEL, "imports='prop.properties: : '");
			GeneratorAsserts.verify(rg, 2, a, 0);
			...
			a.clear();
			ws.setProperty(Constants.REPORT_PROJECT_MODEL, "imports='c ,:c'");
			GeneratorAsserts.verify(rg, 1, a, 0);
			
			a.clear();
			ws.addBasicPlugin(new ProjectContentsEntryPlugin());
			ws.setProperty(Constants.REPORT_PROJECT_MODEL, "includes='settings'");
			GeneratorAsserts.verify(rg, 1, a, 0);
			
			a.clear();
			ws.addBasicPlugin(new ProjectContentsEntryPlugin());
			ws.setProperty(Constants.REPORT_PROJECT_MODEL, "@a=cool");
			GeneratorAsserts.verify(rg, 3, a, 0);
		}
	}
	
	public void testProjectReportGen()
	throws Exception {
		...
		try (ProjectReportGenerator rg = new ProjectReportGenerator(ws)) {
			...
			ws.setProperty(Constants.REPORT_PROJECT, "test2.json;template:=test.xslt;param1=test");
