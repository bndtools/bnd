...
public class WorkspaceReportGeneratorTest extends TestCase {
	...
	private Workspace getWorkspacePlus()
	throws Exception {
	...
	public void testWorkspaceReportModel()
	throws Exception {
		final Workspace ws = getWorkspace();
		...
		try (WorkspaceReportGenerator rg = new WorkspaceReportGenerator(ws)) {
			...
			a.clear();
			ws.setProperty(Constants.REPORT_WORKSPACE_MODEL, "excludes='settings'");
			GeneratorAsserts.verify(rg, 0, a, 0);
			
			a.clear();
			ws.setProperty(Constants.REPORT_WORKSPACE_MODEL, "imports='notFound'");
			GeneratorAsserts.verify(rg, 1, a, 0);
			
			a.clear();
			ws.setProperty(Constants.REPORT_WORKSPACE_MODEL, "imports='build.bnd:prop:properties'");
			GeneratorAsserts.verify(rg, 2, a, 0);
			
			a.clear();
			ws.setProperty(Constants.REPORT_WORKSPACE_MODEL, "imports='build.bnd::properties'");
			GeneratorAsserts.verify(rg, 2, a, 0);
			
			a.clear();
			ws.setProperty(Constants.REPORT_WORKSPACE_MODEL, "imports='prop.properties: : '");
			GeneratorAsserts.verify(rg, 2, a, 0);
			...
			a.clear();
			ws.setProperty(Constants.REPORT_WORKSPACE_MODEL, "imports='c ,:c'");
			GeneratorAsserts.verify(rg, 1, a, 0);
			
			a.clear();
			ws.addBasicPlugin(new WorkspaceContentsEntryPlugin());
			ws.setProperty(Constants.REPORT_WORKSPACE_MODEL, "includes='settings'");
			GeneratorAsserts.verify(rg, 1, a, 0);
			
			a.clear();
			ws.addBasicPlugin(new WorkspaceContentsEntryPlugin());
			ws.setProperty(Constants.REPORT_WORKSPACE_MODEL, "@a=cool");
			GeneratorAsserts.verify(rg, 3, a, 0);
		}
	}
	
	public void testWorkspaceReportGen()
	throws Exception {
		...
		try (WorkspaceReportGenerator rg = new WorkspaceReportGenerator(ws)) {
			...
			a.clear();
			a.add(ws.getBase().getCanonicalPath() + File.separator + "project1" + File.separator + "test.json");
			ws.setProperty(Constants.REPORT_WORKSPACE, "project1/test.json");
			GeneratorAsserts.verify(rg, 2, a, 1);
			
			a.clear();
			a.add(ws.getBase().getCanonicalPath() + File.separator + "project1" + File.separator + "test2.json");
			a.add(ws.getBase().getCanonicalPath() + File.separator + "project1" + File.separator + "cool.xml");
			ws.getProject("project1").setProperty(Constants.REPORT_PROJECT, "cool.xml");
			ws.setProperty(Constants.REPORT_WORKSPACE, "project1/test2.json;template:=project1/test.xslt;param1=test");
			GeneratorAsserts.verify(rg, 2, a, 2);
		}
	}
}
