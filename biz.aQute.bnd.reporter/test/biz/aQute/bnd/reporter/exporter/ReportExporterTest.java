...
public class ReportExporterTest extends TestCase {
  ...
  public void testUnscopedAvailableReport() {
    ...
    assertEquals(2, ReportExporterBuilder.create().setProcessor(processor).build()
        .getAvailableReportsOf(new Object()).size());
  ...
  public void testScopedAvailableReport() {
    ...
    assertEquals(1, ReportExporterBuilder.create().setProcessor(processor).setScope("scope2")
        .build().getAvailableReportsOf(new Object()).size());
    assertTrue(processor.isOk());
  }
  ...
  public void testBasicExport() throws Exception {
    ...
    manifest.getMainAttributes().putValue("Bundle-SymbolicName", "test");
    final Processor processor = new Processor();

    processor.setProperty(ReportExporterConstants.EXPORT_REPORT_INSTRUCTION, "readme.json");

    final Map<String, Resource> result =
        ReportExporterBuilder.create().setProcessor(processor).build().exportReportsOf(jar);

    assertEquals(1, result.size());
    assertTrue(result.values().stream().findFirst().get().buffer().array().length > 0);
    assertTrue(processor.isOk());
  }
  ...
  public void testExportWithTemplate() throws Exception {
    ...
    final Map<String, Resource> result = ReportExporterBuilder.create().setProcessor(processor)
        .setGenerator(ReportGeneratorBuilder.create()
            .addPlugin(ManifestPlugin.class.getCanonicalName()).build())
        .build().exportReportsOf(jar);

    assertEquals(1, result.size());
    assertEquals("test", new String(result.values().stream().findFirst().get().buffer().array()));
    assertTrue(processor.isOk());
  }
  ...
  public void testExportWithTemplateOverride() throws Exception {
    ...
    assertEquals(1, result.size());
    assertEquals("testOverride",
        new String(result.values().stream().findFirst().get().buffer().array()));
    assertTrue(processor.isOk());
  }
  ...
  public void testExportWithConfigName() throws Exception {
    ...
    processor.setProperty(ReportExporterConstants.EXPORT_REPORT_INSTRUCTION,
        "readme.json;configName=api");

    final Map<String, Resource> result = ReportExporterBuilder.create().setProcessor(processor)
        .setGenerator(
            ReportGeneratorBuilder.create().setProcessor(processor).useCustomConfig().build())
        .build().exportReportsOf(jar);

    assertEquals(1, result.size());
    assertTrue(new String(result.values().stream().findFirst().get().buffer().array())
        .contains("valueTest"));
    assertTrue(processor.isOk());
  }
}
