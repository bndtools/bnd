 ...
 * paths.
 * <p>
 * A code snippet is a sample of source codes with a title and a short description that serves to
 * illustrate a specific code usage. Code snippets can either provide a single snippet or contain a
 * list of steps, each one with a title and a description.
 * 
 */
public class CodeSnippetExtractor {
  ...
  public CodeSnippetExtractor() {
    /*
     * To extend the extractor with new languages (xml, json, ...) add the appropriate readers
     * below.
     */
  ...
  /**
   * Extract a list of code snippet by analyzing source codes contained in the directory paths given
   * in argument.
   * <p>
   * Code snippets are ordered as they appear in the directories (files are first sorted in a
   * lexicographic order).
   * 
   * @param directoryPaths a list of directory paths from which code snippets are looked up.
  ...
  public List<CodeSnippetDTO> extract(final String... directoryPaths) {
    ...
    /* We go through each file and for each of them we extract zero or more snippet */
    sortedFiles.stream().flatMap(file -> extractSnippets(file)).forEach(snippet -> {
      /*
       * There is three cases, the snippet has steps, the snippet is a step in a group or is a
       * single snippet.
       */
      if (snippet.getGroupName() != null) {
        /*
         * If the snippet has steps, we place it in a cache, so later, steps can be added to it. We
         * optionally add previously found steps.
         */
        ...
        /*
         * If the snippet is a step, we add it to the parent snippet, or if not found yet, to a
         * cache.
         */
        if (groupCache.get(snippet.getParentGroup()) != null) {
          ...
          groupProgramCache.get(snippet.getParentGroup()).add(snippet.getCodeSnippetProgram());
        }
      } else {
  ...
  private void addReader(final SnippetReader reader) {
    ...
    reader.getSupportedExtension().forEach(e -> _readers.put(e, reader));
  }
  ...
  /*
   * Return a sorted list of files that match the extractor's available snippet readers capability.
   */
  private List<File> getSupportedFiles(final String[] directoryPaths) {
    final List<File> files = new ArrayList<>(50);
    final String spec =
        _readers.keySet().stream().map(e -> "**/*." + e).reduce("", (p, n) -> p + "," + n);
  ...
  /* Select the appropriate readers and extract zero or more snippet from the file in argument. */
  private Stream<Snippet> extractSnippets(final File file) {
    ...
    if (reader != null) {
      try {
        return reader.read(file).stream();
