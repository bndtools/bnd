...
class JavaSnippetReader extends SnippetReader {
  ...
  private static Pattern snippetPattern =
      Pattern.compile("(?:\\$\\{\\s*snippet)(.*?)(?:\\})", Pattern.DOTALL);
  ...
  JavaSnippetReader() {
    ...
    configuration.setLexicalPreservationEnabled(false); // should be true, waiting JavaParser to
                                                        // export LexicalPreservingPrinter
    _parser = new JavaParser(configuration);

    _printConf = new PrettyPrinterConfiguration().setIndentSize(2).setEndOfLineCharacter("\n");
  }
  ...
  public List<Snippet> read(final File file) throws FileNotFoundException {
    ...
    if (!r.isSuccessful()) {
      throw new RuntimeException("Failed to parser file " + file.getPath() + ". "
          + r.getProblems().stream().map(Problem::toString).reduce("", (p, a) -> p + ", " + a));
    }
    ...
    r.ifSuccessful(cu -> {
      ...
      cu.getTypes().forEach(typeDeclaration -> {
        typeDeclaration.getComment().map(c -> {
          ...
          return result;
            ...
            .forEach(typeConfig -> snippets.add(
                createSnippet(typeDeclaration.getNameAsString(), typeConfig, () -> formatCode(cu,
                    typeDeclaration, typeConfig.includeImports, typeConfig.includeDeclaration))));

        typeDeclaration.getMethods()
            .forEach(methodDeclaration -> methodDeclaration.getComment().map(c -> {
              final List<CodeSnippetConfig> result = parseSnippetConfig(c);
              if (c.getContent().isEmpty()) {
              ...
              return result;
                ...
                methodConfig -> snippets.add(createSnippet(methodDeclaration.getNameAsString(),
                    methodConfig, () -> formatCode(cu, methodDeclaration,
                        methodConfig.includeImports, methodConfig.includeDeclaration)))));
      });
    });
  ...
  private String formatCode(final CompilationUnit cu, final TypeDeclaration<?> node,
      final boolean showImport, final boolean showDeclaration) {
    if (showImport && showDeclaration) {
      final StringBuilder sb = new StringBuilder();

      formatImports(sb, cu);

      return sb.append(node.toString(_printConf)).toString();
    } else if (showImport) {
      final StringBuilder sb = new StringBuilder();
  ...
  private void formatTypeNoDeclaration(final StringBuilder sb, final TypeDeclaration<?> node) {
    if (node.getMembers() != null && !node.getMembers().isEmpty()) {
      node.getMember(0).addMarkerAnnotation("$$$");

      String body = node.toString(_printConf);
      body = Pattern.compile(".*?\\$\\$\\$\n", Pattern.DOTALL).matcher(body).replaceFirst("")
          .replaceAll("\n[^\\S\\r\\n]{2}", "\n");
  ...
  private String formatCode(final CompilationUnit cu, final MethodDeclaration node,
      final boolean showImport, final boolean showDeclaration) {
    if (showImport && showDeclaration) {
      final StringBuilder sb = new StringBuilder();
  ...
  private void formatMethodNoDeclaration(final StringBuilder sb, final MethodDeclaration node) {
    node.getBody().ifPresent(b -> {
      final String body = b.toString(_printConf).replaceAll("\n[^\\S\\r\\n]{2}", "\n");
      sb.append(body.substring(2, body.length() - 2));
    });
  }
  ...
  private void formatImports(final StringBuilder sb, final CompilationUnit cu) {
    if (cu.getImports() != null && !cu.getImports().isEmpty()) {
      cu.getImports().forEach(i -> sb.append(i.toString(_printConf)));
      sb.append("\n");
    }
  }
  ...
  private boolean isLegalConfig(final String unparsedConfig) {
    final Matcher matcher = Pattern.compile("(?<!\\\\)\"").matcher(unparsedConfig);
