 ...
 * prefixed by "ws-" defined in the build.bnd file. Properties take precedence.
 */
...
public class CommonInfoPlugin implements ReportEntryPlugin<Workspace>, Plugin {
  ...
  public CommonInfoDTO extract(final Workspace workspace, final Locale locale) throws Exception {
    ...
    commonInfo.contactAddress = extractHeader(Constants.BUNDLE_CONTACTADDRESS, "contactAddress",
        workspace, HeadersHelper::convertBundleContactAddress);
    ...
    commonInfo.icons =
        extractHeader(Constants.BUNDLE_ICON, "icons", workspace, HeadersHelper::convertBundleIcons);
    commonInfo.licenses = extractHeader(Constants.BUNDLE_LICENSE, "licenses", workspace,
        HeadersHelper::convertBundleLicenses);
    commonInfo.name =
        extractHeader(Constants.BUNDLE_NAME, "name", workspace, HeadersHelper::convertBundleName);
    commonInfo.scm =
        extractHeader(Constants.BUNDLE_SCM, "scm", workspace, HeadersHelper::convertBundleSCM);
    commonInfo.updateLocation = extractHeader(Constants.BUNDLE_UPDATELOCATION, "updateLocation",
        workspace, HeadersHelper::convertBundleUpdateLocation);
  ...
  private <T> T extractHeader(final String header, final String propertyName,
      final Workspace workspace, final Function<Parameters, T> converter) {
