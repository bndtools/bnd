 ...
 * DTO. The {@link ImportResourcePlugin#URL_PROPERTY} property must be set and the
 * {@link ImportResourcePlugin#TYPE_PROPERTY} can be set if the file type is not obvious (derived
 * from its extension).
 */
...
public class ImportResourcePlugin implements ReportEntryPlugin<Object>, Plugin, RegistryPlugin {
  ...
  private Object convertFile(final InputStream inputStream, final String extension)
    ...
    _reporter.error("Unable to convert the imported file %s of type %s, available types are: %s",
        getUrl(), getType(), getAvailableConverterExtensions());
    return null;
  }
