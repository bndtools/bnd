...
public class ManifestPlugin implements ReportEntryPlugin<Jar>, Plugin {
  ...
  public OSGiHeadersDTO extract(final Jar jar, final Locale locale) {
    ...
    if (manifest != null) {
      ...
      headers.bundleName =
          HeadersHelper.convertBundleName(manifest.getHeader(Constants.BUNDLE_NAME, false));
      headers.bundleDescription = HeadersHelper
          .convertBundleDescription(manifest.getHeader(Constants.BUNDLE_DESCRIPTION, false));
      headers.bundleVersion =
          HeadersHelper.convertBundleVersion(manifest.getHeader(Constants.BUNDLE_VERSION, false));
      headers.bundleCategories = HeadersHelper
          .convertBundleCategories(manifest.getHeader(Constants.BUNDLE_CATEGORY, false));
      headers.bundleIcons =
          HeadersHelper.convertBundleIcons(manifest.getHeader(Constants.BUNDLE_ICON, false));
      headers.bundleDocURL =
          HeadersHelper.convertBundleDocURL(manifest.getHeader(Constants.BUNDLE_DOCURL, false));
      ...
      headers.bundleLicenses =
          HeadersHelper.convertBundleLicenses(manifest.getHeader(Constants.BUNDLE_LICENSE, false));
      headers.bundleDevelopers = HeadersHelper
          .convertBundleDevelopers(manifest.getHeader(Constants.BUNDLE_DEVELOPERS, false));
      headers.bundleSCM =
          HeadersHelper.convertBundleSCM(manifest.getHeader(Constants.BUNDLE_SCM, false));
      headers.bundleCopyright = HeadersHelper
          .convertBundleCopyright(manifest.getHeader(Constants.BUNDLE_COPYRIGHT, false));
      headers.bundleVendor =
          HeadersHelper.convertBundleVendor(manifest.getHeader(Constants.BUNDLE_VENDOR, false));
      ...
      headers.importPackages =
          HeadersHelper.convertImportPackages(manifest.getHeader(Constants.IMPORT_PACKAGE, false));
      ...
      headers.exportPackages =
          HeadersHelper.convertExportPackages(manifest.getHeader(Constants.EXPORT_PACKAGE, false));
      ...
      headers.requireBundles =
          HeadersHelper.convertRequireBundles(manifest.getHeader(Constants.REQUIRE_BUNDLE, false));
      ...
      headers.fragmentHost =
          HeadersHelper.convertFragmentHost(manifest.getHeader(Constants.FRAGMENT_HOST, false));
      ...
      headers.bundleManifestVersion = HeadersHelper.convertBundleManifestVersion(
          manifest.getHeader(Constants.BUNDLE_MANIFESTVERSION, false));
