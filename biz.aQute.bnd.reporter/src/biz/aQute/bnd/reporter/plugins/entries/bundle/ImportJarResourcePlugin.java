 ...
 * into DTO. The {@link ImportJarResourcePlugin#PATH_PROPERTY} property must be set and the
 * {@link ImportJarResourcePlugin#TYPE_PROPERTY} can be set if the file type is not obvious (derived
 * from its extension).
 */
...
public class ImportJarResourcePlugin implements ReportEntryPlugin<Jar>, Plugin, RegistryPlugin {
  ...
  private Object convertFile(final InputStream inputStream, final String extension)
      throws Exception {
    for (final ResourceConverterPlugin deserializer : _registry
        .getPlugins(ResourceConverterPlugin.class)) {
      if (ArrayHelper.containsIgnoreCase(deserializer.getHandledExtensions(), extension)) {
        return deserializer.extract(inputStream);
      }
    }
    _reporter.error("unable to convert the imported file %s of type %s, available types are: %s",
        getPath(), getType(), getAvailableConverterExtensions());
    return null;
  }

  private String[] getAvailableConverterExtensions() {
    return _registry.getPlugins(ResourceConverterPlugin.class).stream()
