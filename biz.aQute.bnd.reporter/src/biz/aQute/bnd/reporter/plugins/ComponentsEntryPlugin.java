...
import biz.aQute.bnd.reporter.plugins.dto.ComponentDescriptionDTO;
import biz.aQute.bnd.reporter.plugins.dto.ReferenceDTO;
...
public class ComponentsEntryPlugin implements BundleEntryPlugin {

	final private static String COMPONENTS_TAG = "components";
			...
			if (r != null) {
				try (InputStream in = r.openInputStream()) {
					...
					if (implementations.getLength() > 0
						...
						reporter.warning("the component does not declare an implementation class %s", path);
					}
					...
					for (int i = 0; i < propertiesFile.getLength(); i++) {
						...
						if (pathProp.isEmpty()) {
							reporter.warning("missing property path attribute for component %s", path);
						} else {
							...
							if (rP != null) {
								try (InputStream inProp = rP.openInputStream()) {
									...
									for (final String key : prop.stringPropertyNames()) {
										final TypedAttributeDTO p = new TypedAttributeDTO();
										p.name = key;
										p.type = "String";
										p.values.add(prop.getProperty(key));

										comp.properties.add(p);

										propertiesName.add(key);
									}
								}
							} else {
								reporter.warning("cannot find property file at path %s for component %s", pathProp,
										path);
							}
						}
					}
					...
					for (int i = 0; i < properties.getLength(); i++) {
						...
						if (property.hasAttribute(NAME_ATTR)) {
							...
							pName = "!! MISSING !!";
							reporter.warning("missing property name attribute for component %s", path);
						}
					...
					for (int i = 0; i < references.getLength(); i++) {
						...
						if (reference.hasAttribute(INTERFACE_ATTR)) {
							...
							reporter.warning("missing reference interface attribute for component %s", path);
						}
						...
						if (reference.hasAttribute(NAME_ATTR)) {
							...
							ref.name = "!! MISSING !!";
							reporter.warning("missing reference name attribute for component %s", path);
						}
				...
				if (!path.contains("*")) {
					reporter.warning("xml component file not found at path %s", path);
				}
			}
		}
		return comps;
