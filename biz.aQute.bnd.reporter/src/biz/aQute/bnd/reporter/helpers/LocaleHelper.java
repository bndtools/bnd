...
public class LocaleHelper {
  ...
  /**
   * If the argument is a variable, its corresponding value will be returned for the default locale
   * of this helper instance. Otherwise, the argument is returned.
   * <p>
   * Values will be search from the most specific to the less specific locale including unlocalized
   * (empty locale) value.
   *
  ...
  /**
   * If the argument is a variable, its corresponding value will be returned for the specified
   * locale. Otherwise, the argument is returned.
   * <p>
   * Values will be search from the most specific to the less specific locale including unlocalized
   * (empty locale) value.
   *
      ...
      if (variableOrValue.startsWith("%")) {
        ...
        do {
          if (_localizations.get(nextLocale.toString()) != null) {
            result = _localizations.get(nextLocale.toString()).get(variable);
          }
  ...
  private static boolean hasLocalization(final Jar jar, final String path) {
    return jar.getResources().keySet().stream().anyMatch(k -> k.startsWith(path));
  }

  private Map<String, Map<String, String>> extractLocalizations(final Jar jar, final String path) {
    final Map<String, Map<String, String>> result = new HashMap<>();

    jar.getResources().entrySet().stream().filter(e -> e.getKey().startsWith(path))
        .forEach(entry -> {
