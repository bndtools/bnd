...
public class VersionHelper {
   ...
   * @return the version range DTO or {@code null} if the version range in argument is {@code null}
  ...
  static public VersionRangeDTO toRange(final VersionRange versionRange) {
    ...
    result.floor.major = versionRange.getLow().getMajor();
    result.floor.minor = Integer.valueOf(versionRange.getLow().getMinor());
    result.floor.micro = Integer.valueOf(versionRange.getLow().getMicro());
    if (versionRange.getLow().getQualifier() != null
        && !versionRange.getLow().getQualifier().isEmpty()) {
      result.floor.qualifier = versionRange.getLow().getQualifier();
    }

    if (!versionRange.isSingleVersion()) {
      result.ceiling = new VersionInRangeDTO();
      result.ceiling.include = versionRange.includeHigh();
      result.ceiling.major = versionRange.getHigh().getMajor();
      result.ceiling.minor = Integer.valueOf(versionRange.getHigh().getMinor());
      result.ceiling.micro = Integer.valueOf(versionRange.getHigh().getMicro());
      if (versionRange.getHigh().getQualifier() != null
          && !versionRange.getHigh().getQualifier().isEmpty()) {
        result.ceiling.qualifier = versionRange.getHigh().getQualifier();
      }
    }
   ...
   * @return the version DTO or {@code null} if the version in argument is {@code null} or is not a
   *         version
   */
