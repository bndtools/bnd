...
public class P2IndexerTest extends TestCase {
	...
	@Test
	public void testRecreateP2IndexCacheIfResourceURLsNotFitsRepoURL() throws Exception {
		Path testdateDir = Paths.get(System.getProperty("user.dir"), "testdata");

		Path invalidIndex = testdateDir.resolve("index-p2-starts-with-host.xml");
		Assert.assertTrue(invalidIndex.toFile().exists());
		Assert.assertTrue(new String(Files.readAllBytes(invalidIndex))
				.contains("http://starts-with-host/plugins/dummy_1.0.0.jar"));
		...
		// good case: valid p2 cache
		try (P2Indexer indexer = new P2Indexer(new Slf4jReporter(), invalidCache.toFile(), new HttpClient(),
				new URI("http://starts-with-host/plugins/"), "valid")) {}
		byte[] md5Actual = md5(invalidCache.resolve("index.xml.gz"));
		Assert.assertTrue(Arrays.areEqual(md5, md5Actual));

		// bad case: invalid p2 cache
		Exception expectedException = null;
		try (P2Indexer indexer = new P2Indexer(new Slf4jReporter(), invalidCache.toFile(), new HttpClient(),
				new URI("http://starts-not-with-host/plugins/"), "invalid")) {}
		catch (Exception e) {
			expectedException = e;
		}
		Assert.assertNotNull(expectedException);
		Assert.assertEquals(InvocationTargetException.class, expectedException.getClass());
	}
