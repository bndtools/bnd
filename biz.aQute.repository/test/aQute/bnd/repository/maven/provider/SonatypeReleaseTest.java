...
public class SonatypeReleaseTest {
	...
	File		sonatypeReleaseRepoFile	= new File(wsDir, "cnf/ext/sonatype_release.bnd");
	...
	public void prepareTest() throws IOException {
		...
		assumeTrue(hasRequiredEnvironmentVariables(), 
			"Skipping test: Required environment variables (SONATYPE_USERNAME, SONATYPE_BEARER, GPG_PASSPHRASE) are not set");
		...
		sonatypeReleaseRepoFile.delete();
		assertFalse(sonatypeReleaseRepoFile.exists());
	...
	private boolean hasRequiredEnvironmentVariables() {
		String sonatypeUsername = System.getenv("SONATYPE_USERNAME");
		String sonatypeBearer = System.getenv("SONATYPE_BEARER");
		String gpgPassphrase = System.getenv("GPG_PASSPHRASE");
		
		return sonatypeUsername != null && !sonatypeUsername.isEmpty()
			&& sonatypeBearer != null && !sonatypeBearer.isEmpty()
	...
	public void testReleaseDeployment() throws Exception {
		try (Workspace ws = Workspace.findWorkspace(wsDir)) {
			new File(wsDir, "cnf/ext/sonatype_release.bnd_").renameTo(sonatypeReleaseRepoFile);
			assertTrue(sonatypeReleaseRepoFile.exists());
		}
		testDeployment();
	}
	...
	public void testStagingDeployment() throws Exception {
		try (Workspace ws = Workspace.findWorkspace(wsDir)) {
			new File(wsDir, "cnf/ext/sonatype_staging.bnd_").renameTo(sonatypeReleaseRepoFile);
			assertTrue(sonatypeReleaseRepoFile.exists());
		}
		testDeployment();
	}
	...
	private void testDeploymentStatusCheck(Workspace ws, String deploymentId) throws Exception {
		...
		String statusUrl = MavenBndRepository.CENTRAL_SONATYPE_PUBLISHER_URL + "/status?id=" + deploymentId;
	...
	private void dropDeployment(HttpClient client, String deploymentId) throws Exception {
		String dropUrl = MavenBndRepository.CENTRAL_SONATYPE_PUBLISHER_URL + "/deployment/" + deploymentId;
