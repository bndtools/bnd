...
public class FeatureParserTest {
	...
	/**
	 * Test that the osgi.identity requirements created by toResource() carry
	 * the provenance/relation attributes needed by the -buildpath feature
	 * expansion: bnd.relation, id, version, type and the os/ws/arch platform
	 * attributes. The filter directives must remain untouched.
	 */
	@Test
	public void testMemberRelationAttributes() throws Exception {
		String featureXml = """
			<?xml version="1.0" encoding="UTF-8"?>
			<feature id="test.feature" label="Test" version="1.0.0" provider-name="Test">
			   <plugin id="plain.plugin" version="1.2.3" unpack="false"/>
			   <plugin id="platform.plugin" version="2.0.0" os="win32,linux" ws="gtk" arch="x86_64" fragment="true"/>
			   <includes id="included.feature" version="3.0.0"/>
			   <includes id="optional.feature" version="4.0.0" optional="true"/>
			   <requires>
			      <import plugin="required.plugin" version="1.0.0"/>
			      <import feature="required.feature" version="2.0.0"/>
			   </requires>
		...
		try (InputStream is = IO.stream(featureJar)) {
			...
			Requirement plainPlugin = requirementById(requirements, "plain.plugin");
			softly.assertThat(plainPlugin.getAttributes())
				.containsEntry(Feature.RELATION_ATTRIBUTE, Feature.RELATION_PLUGIN)
				.containsEntry("id", "plain.plugin")
				.doesNotContainKeys("os", "ws", "arch", "fragment",
					IdentityNamespace.CAPABILITY_TYPE_ATTRIBUTE);
			softly.assertThat(String.valueOf(plainPlugin.getAttributes()
				.get("version")))
				.isEqualTo("1.2.3");
			softly.assertThat(plainPlugin.getDirectives()
				.get("filter"))
				.isEqualTo("(&(osgi.identity=plain.plugin)(version=1.2.3))");

			Requirement platformPlugin = requirementById(requirements, "platform.plugin");
			softly.assertThat(platformPlugin.getAttributes())
				.containsEntry(Feature.RELATION_ATTRIBUTE, Feature.RELATION_PLUGIN)
				.containsEntry("os", "win32,linux")
				.containsEntry("ws", "gtk")
				.containsEntry("arch", "x86_64")
				.containsEntry("fragment", "true");

			Requirement include = requirementById(requirements, "included.feature");
			softly.assertThat(include.getAttributes())
				.containsEntry(Feature.RELATION_ATTRIBUTE, Feature.RELATION_INCLUDE)
				.containsEntry("id", "included.feature")
				.containsEntry(IdentityNamespace.CAPABILITY_TYPE_ATTRIBUTE, "org.eclipse.update.feature");
			softly.assertThat(include.getDirectives()
				.get("filter"))
				.isEqualTo("(&(osgi.identity=included.feature)(type=org.eclipse.update.feature)(version=3.0.0))");
			softly.assertThat(include.getDirectives())
				.doesNotContainKey("resolution");

			Requirement optionalInclude = requirementById(requirements, "optional.feature");
			softly.assertThat(optionalInclude.getAttributes())
				.containsEntry(Feature.RELATION_ATTRIBUTE, Feature.RELATION_INCLUDE);
			softly.assertThat(optionalInclude.getDirectives())
				.containsEntry("resolution", "optional");

			List<Requirement> requireReqs = requirements.stream()
				.filter(r -> Feature.RELATION_REQUIRE.equals(r.getAttributes()
					.get(Feature.RELATION_ATTRIBUTE)))
				.toList();
			softly.assertThat(requireReqs)
				.as("requires imports must be marked as require relation")
				.hasSize(2);
		}
	}

	private static Requirement requirementById(List<Requirement> requirements, String id) {
		return requirements.stream()
			.filter(r -> id.equals(r.getAttributes()
				.get("id"))
				|| String.valueOf(r.getDirectives()
					.get("filter"))
					.contains("(osgi.identity=" + id + ")"))
			.findFirst()
			.orElseThrow(() -> new AssertionError("No requirement found for " + id));
	}
}
